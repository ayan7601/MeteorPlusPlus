...
public class MyScreen extends WindowScreen {
    ...
    public static void checkVersionOnServerJoin() {
        ...
        MeteorExecutor.execute(() -> {
            try {
                String versionString = Http.get("https://meteorplusplusclient.com/versions/normal1.21.8.txt").sendString();
                if (versionString != null && !versionString.isEmpty()) {
                    int latestVersion = Integer.parseInt(versionString.trim());


                    if (latestVersion > MeteorPlusPlusAddon.MyScreenVERSION) {
        ...
        MeteorExecutor.execute(() -> {
            try {
                ...
                if (versionString != null && !versionString.isEmpty()) {
                    ...
                    latestVersion = MeteorPlusPlusAddon.MyScreenVERSION;
                }
    ...
    private void buildUI() {
        addWrappedMessage("Welcome to MeteorPlusPlus Client version checker. Here you can see your current version and check for updates.");

        add(theme.horizontalSeparator()).padVertical(theme.scale(4)).expandX();

        addMessage(String.format("Installed Version: %d", MeteorPlusPlusAddon.MyScreenVERSION));
        ...
        if (isVersionFetched && latestVersion > MeteorPlusPlusAddon.MyScreenVERSION) {
            addWrappedMessage("You're using an outdated version of the MeteorPlusPlus addon. Please update to the latest version. Newer versions may include important bug fixes, improvements, and additional features.");
        } else if (isVersionFetched && latestVersion == MeteorPlusPlusAddon.MyScreenVERSION) {
            addWrappedMessage("You're using the latest version of the MeteorPlusPlus addon. No update needed.");
        } else {
        ...
        githubButton.action = () -> {
            Util.getOperatingSystem().open("https://github.com/realnnpg/meteorplusplus");
        };

        WButton websiteButton = buttonsContainer.add(theme.button("Website")).expandX().widget();
        websiteButton.action = () -> {
            Util.getOperatingSystem().open("https://meteorplusplusclient.com");
        };
    }
