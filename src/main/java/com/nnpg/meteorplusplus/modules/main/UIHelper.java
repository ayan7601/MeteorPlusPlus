...
import com.nnpg.meteorplusplus.utils.meteorplusplus.StringUtils;
...
public class UIHelper extends Module {
    ...
    public UIHelper() {
        super(MeteorPlusPlusAddon.CATEGORY,
                "ui-helper",
    ...
    private void onSendPacket(PacketEvent.Send event) {
        ...
        if (enableAutoAdvance.get()) {
            if (event.packet instanceof ClickSlotC2SPacket packet) {
                if (packet.slot() >= 0 && StringUtils.convertUnicodeToAscii(
                        mc.player.currentScreenHandler.getSlot(packet.slot()).getStack().getName().getString()).equals("drop loot")) {
