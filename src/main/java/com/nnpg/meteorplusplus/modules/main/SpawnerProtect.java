...
public class SpawnerProtect extends Module {
    ...
    public SpawnerProtect() {
        super(MeteorPlusPlusAddon.CATEGORY, "spawner-protect", "Breaks spawners and secures them when a player is detected");
    }
    ...
    private void handleGoingToSpawners() {
        ...
        if (mc.crosshairTarget instanceof BlockHitResult hit && hit.getBlockPos().equals(currentTarget)) {
            ...
            if (com.nnpg.meteorplusplus.VersionUtil.getSelectedSlot(mc.player) != pickaxe.slot()) {
    ...
    private void handleDisconnecting() {
        ...
        if (mc.world != null) mc.world.disconnect(net.minecraft.client.world.ClientWorld.QUITTING_MULTIPLAYER_TEXT);
        this.toggle();
    }
    ...
    private void sendWebhookNotification() {
        ...
        String payload = String.format("""
                {
                    "username": "MeteorPlusPlus Webhook",
                    ...
                    "embeds": [{
                        ...
                        "color": %d,
                        "timestamp": "%s",
                        "footer": { "text": "Sent by MeteorPlusPlus" }
                    }]
                }""",
